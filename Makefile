...
personal-website-pipeline:
	...
	-fly -t homelab set-pipeline -n -p personal-website -c compiled_pipelines/personal_website.yml -l common/vars/secrets.dec.yml -l common/vars/vars.yml
...
k8s-homelab-helm-repo-pipeline:
	...
	-fly -t homelab set-pipeline -n -p k8s-homelab -c compiled_pipelines/k8s_homelab.yml -l common/vars/secrets.dec.yml -l common/vars/vars.yml
	make clean-decrypted-files

images-build-pipeline:
	make decrypt-secrets
	cat common/reusable_blocks.yml image_build/image_build.yml > compiled_pipelines/image_build.yml
	-fly -t homelab set-pipeline -n -p images-build -c compiled_pipelines/image_build.yml -l common/vars/secrets.dec.yml -l common/vars/vars.yml
	make clean-decrypted-files
